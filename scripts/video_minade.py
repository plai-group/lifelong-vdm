...
from improved_diffusion.video_datasets import get_eval_dataset, SpacedBaseDataset, ChunkedBaseDataset
from improved_diffusion.test_util import parse_eval_run_identifier, SampleDataset
...
def compute_color_transition_stats(sample_colors, n_obs):
    """
    Estimate, per seed, the probability of each color-to-color transition, counting only
    transitions in the generated part of the video (t >= n_obs). Colors are indexed red/yellow/green.
    - sample_colors shape: <n_seeds x n_timesteps x n_balls>
    - output shape: <n_seeds x 3 x 3> (row-normalised)
    """
...
def get_kernel_maxacts(frame, kernels, n_max=1):
    ...
    for k, kernel in enumerate(kernels):
        ...
        # HACK: Keep the best match plus only candidates >4 px away from it, so one ball is not reported n_max times.
...
def compute_minADE(test_dataset, sample_datasets, n_obs, T, n_balls):
    ...
    kernel_width = 8
    [I, J] = np.meshgrid(ar(-1.25, 1.25, 2.5/kernel_width), ar(-1.25, 1.25, 2.5/kernel_width))  # 8x8 grid over the ball's extent
    # 8x8 Gaussian bump matching the ball shape in datasets/ball.py; one 3x8x8 RGB kernel per color below
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_videos", type=int, required=True, help="Number of test videos to evaluate on (how many video_sample.py generated).")
    ...
    # Load the ground-truth test videos to compare the samples against
