...
class SpacedEgoLifeDataset(ContinuousEgoLifeDataset):
    ...
    def _get_start_frame_index(self, idx):
        return self.frame_range[0] + int(self.start_indices[idx])
    ...
    def _initialize_file_index_mapping(self):
        ...
        total = self.frame_range[1] - self.frame_range[0]
        spacing = total // self.n_data
        if spacing >= self.window_length:
            # old behavior: evenly-spaced non-overlapping windows, aligned to window_length
            spacing = spacing - spacing % self.window_length
            self.start_indices = np.arange(self.n_data) * spacing
            print(f"Total Frames: {self.total_frames}, Selected Frames: {total}, "
                  f"Spacing: {spacing}, # Data: {self.n_data}")
        else:
            # overlapping windows: use linspace over actual frame range to avoid OOB on last file
            actual_last_size = torch.load(self.pt_files[-1], map_location='cpu')['quantized_latents'].shape[0]
            actual_total = (len(self.pt_files) - 1) * 500 + actual_last_size
            total = min(self.frame_range[1], actual_total) - self.frame_range[0]
            assert total >= self.window_length, \
                f"Not enough frames ({total}) for window_length={self.window_length}"
            self.start_indices = np.linspace(0, total - self.window_length, self.n_data)
            avg_stride = (total - self.window_length) / max(self.n_data - 1, 1)
            print(f"Total Frames: {actual_total}, Selected Frames: {total}, "
                  f"Avg Stride: {avg_stride:.2f}, Window: {self.window_length}, # Data: {self.n_data}")


if __name__ == "__main__":
    dataset_path = "datasets/egolife"
    output_video_folder = "datasets/egolife/test_decoded"
    use_fp16 = True
